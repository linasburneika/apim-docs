...
1. Go [here](/apis/itwin-entities/operations/create-entity/)
...
The iTwin Entities API is used to create and manage entities. A POST call to the entities endpoint will create a new entity for the iTwin specified by the `iTwinId`. You need to replace the `ITWIN_ID` with the your iTwinId in the request as mentioned in the [Prerequisites](#prerequisites) section.

At minimum, `class` and `code` are required for creating a new entity. The `code` property should be unique. If you already have a `federationId` for the entity also specify it. Otherwise, a new `federationId` would be auto-generated for the new entity. If you get a conflict error, then you must replace the `federationId` with a unique value. See the [iTwin Entities API documentation](/apis/itwin-entities/operations/create-entity/) for descriptions of the different properties, including default values.
...
The `federationId` of an entity can be used to [query for a specific entity](/apis/itwin-entities/operations/get-entity/). In addition to the `federationId`, you need to know the `iTwinId` for the iTwin to which the queried entity belongs.
...
Similarly you can use the `name` or `class` paramaters (or a combination of any of the rest of the query parameter listed for the [get entities API](/apis/itwin-entities/operations/get-entities/)) to search for entities by these values.
